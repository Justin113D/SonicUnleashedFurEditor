...
from .utilities import ADDON_DIR
from .exceptions import SUFEUserException


def _get_geometry_nodes(include_editor):

    templates_path = os.path.join(
        ADDON_DIR, "GeometryNodes.blend")

    with bpy.data.libraries.load(templates_path, link=True) as (data_from, data_to):

        if include_editor:
            data_to.node_groups = [
                "SUFE_Shells", "SUFE_Editor"]
        else:
            data_to.node_groups = ["SUFE_Shells"]
...
class BaseUnleashedFurOperator(BaseOperator):
    bl_options = {'UNDO'}
    ...
    def _setup_shell_modifier(obj: bpy.types.Object, node_tree: bpy.types.NodeTree):
        ...
        modifier: bpy.types.NodesModifier = modifiers.new(
            "Sonic Unleashed Fur Shells", "NODES")
...
class SUFE_OT_AddShells(BaseUnleashedFurOperator):
    bl_idname = "sufe.addshells"
    bl_label = "Add Sonic Unleashed Fur Shells"
...
class SUFE_OT_AddEditor(BaseUnleashedFurOperator):
    bl_idname = "sufe.addeditor"
    bl_label = "Add Sonic Unleashed Fur Editor"
    ...
    def _setup_editor_modifier(obj: bpy.types.Object, node_tree: bpy.types.NodeTree, shell_modifier_index):
        ...
        if not has_modifier:
            index = len(modifiers)
            modifier = modifiers.new("Sonic Unleashed Fur Shell Editor", "NODES")
            modifier.node_group = node_tree
            modifier[_find_input_socket(
...
class SUFE_OT_SwitchVertexColors(BaseOperator):
    bl_idname = "sufe.switchvertexcolors"
    ...
    def _execute(self, context):
        ...
        if self.attribute_name not in color_attribs:
            raise SUFEUserException(f"Mesh has no color attribute \"{self.attribute_name}\"! Please set up the fur editor!")
        color_attribs.active_color_name = self.attribute_name
        return {'FINISHED'}


class SUFE_OT_SetBrushDir(BaseOperator):
    bl_idname = "sufe.setbrushdir"
    ...
    def _execute(self, context):
        context.scene.sufe_settings.vertex_paint_direction = (
            self.direction[0],
            self.direction[1],
            self.direction[2]
        )
