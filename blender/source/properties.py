...
def _check_older_than_5():
    return bpy.app.version[0] < 5

def _get_unified_paint_settings(scene: bpy.types.Scene):
    if hasattr(scene.tool_settings, "unified_paint_settings"): # < 5.0 support
        return scene.tool_settings.unified_paint_settings
    else:
        return scene.tool_settings.vertex_paint.unified_paint_settings

def _get_brush_color():
    scene = bpy.context.scene
    unified_paint_settings = _get_unified_paint_settings(scene)

    if unified_paint_settings.use_unified_color:
        result = unified_paint_settings.color
    else:
        result = scene.tool_settings.vertex_paint.brush.color

    if _check_older_than_5():
        return Vector((
...
def _set_brush_color(value):
    ...
    if unified_paint_settings.use_unified_color:
        unified_paint_settings.color = value
    else:
...
class SUFE_Settings(bpy.types.PropertyGroup):
    ...
    @classmethod
    def register(cls):
        bpy.types.Scene.sufe_settings = PointerProperty(
            type=SUFE_Settings)
